			...
			for (int x = 0; x < data.width; ++x)
			{
				...
				if (tile_id > 0)
				{
					...
					if (tileset != NULL)
					{
						...
						if (item->data->properties.GetProperty("Draw") || App->input->debug)
...
TileSet* j1Map::GetTilesetFromTileId(int id) const
{
	...
	while (item != NULL)
	{
		if (item->data->firstgid + item->data->tilecount-1 >= id && item->data->firstgid <= id)
		{
			return item->data;
		}
		item = item->next;
...
bool j1Map::LoadProperties(pugi::xml_node& node, Properties& properties)
{
	...
	{
		properties.property.PushBack(prop.attribute("name").as_string());
		properties.value.PushBack(prop.attribute("value").as_int());
	}
